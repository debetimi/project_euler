(ns project-euler.p018
  (:require [clojure.string :as s]))

;;; By starting at the top of the triangle below and moving to adjacent numbers on the row below, the maximum total from top to bottom is 23.
;;; 
;;; 3
;;; 7 4
;;; 2 4 6
;;; 8 5 9 3
;;; 
;;; That is, 3 + 7 + 4 + 9 = 23.
;;; 
;;; Find the maximum total from top to bottom of the triangle below:
;;; 
;;; 75
;;; 95 64
;;; 17 47 82
;;; 18 35 87 10
;;; 20 04 82 47 65
;;; 19 01 23 75 03 34
;;; 88 02 77 73 07 63 67
;;; 99 65 04 28 06 16 70 92
;;; 41 41 26 56 83 40 80 70 33
;;; 41 48 72 33 47 32 37 16 94 29
;;; 53 71 44 65 25 43 91 52 97 51 14
;;; 70 11 33 28 77 73 17 78 39 68 17 57
;;; 91 71 52 38 17 14 91 43 58 50 27 29 48
;;; 63 66 04 68 89 53 67 30 73 16 69 87 40 31
;;; 04 62 98 27 23 09 70 98 73 93 38 53 60 04 23

(def pyramid 
"75
95 64
17 47 82
18 35 87 10
20 04 82 47 65
19 01 23 75 03 34
88 02 77 73 07 63 67
99 65 04 28 06 16 70 92
41 41 26 56 83 40 80 70 33
41 48 72 33 47 32 37 16 94 29
53 71 44 65 25 43 91 52 97 51 14
70 11 33 28 77 73 17 78 39 68 17 57
91 71 52 38 17 14 91 43 58 50 27 29 48
63 66 04 68 89 53 67 30 73 16 69 87 40 31
04 62 98 27 23 09 70 98 73 93 38 53 60 04 23")


;; The above pyramid as a list of strings represent each row.
;; starting from the bottom row to the top
(def info (reverse (clojure.string/split pyramid #"\n")))

(defn- remove-leading-zeros 
  "Since 09 is not a valid number in clojure, this helper function removes
   leading zeros from single digit numbers"
  [string]
  (s/replace string #"0(\d)" "$1"))

(defn str-row->int-row 
  "Converts a string repesenting a list of integers into a list of integers
   '01 13 45 67' -> (1 13 45 67)"
  [str-row]
 (map (comp read-string remove-leading-zeros) (s/split str-row #" ")))

(defn max-siblings
  "Takes a list of integers and list of maximum child of each of those integers
   in the tree and returns a list of the maximum of any pair of siblings"
  [max-child row]
  (if (not= 1 (count row)) 
    (map (partial reduce max) (partition 2 1 (map + row max-child)))
    (map + max-child row)))

(defn solve [] (reduce max-siblings (repeat 0) (map str-row->int-row info)))
