...
(def direction-functions 
  {:right (fn [coord] 
            [(inc (first coord)) (second coord)])
   :down (fn [coord] 
           [(first coord) (inc (second coord))]) 
   :rdiag (fn [coord]
            (into [] (map inc coord)))
   :ldiag (fn [coord]
            [(dec (first coord)) (inc (second coord))])})
...
(defn greatest-product
  ...
  (letfn [(dir->val [dir] 
    ...
    (apply max (map dir->val (vals direction-functions)))))

(defn solve
  ([]
   (solve 4))
  ([n]
  (apply max (map (partial greatest-product n) (keys coord->value)))))
